    ...
    <div class="card-body">
        <form action="{{ route('anggota.edit.post') }}" method="POST">
            ...
            <div class="form-group">
                ...
                <input type="text" name="nia" value="{{ $anggota->nia }}" id="inputnia" class="form-control form-control-sm">
            </div>
            ...
            <div class="form-group row">
                ...
                <div class="col-sm-6">
                    ...
                    <select name="goldar" id="goldar" class="form-control form-control-sm">
                        <option value="none"
                            {{ ($anggota->goldar == 'NULL') ? 'selected' : '' }}><i>--none--</i></option>
                        <option value="A+"
                            {{ ($anggota->goldar == 'A+') ? 'selected' : '' }}>A+</option>
                        <option value="A-"
                            {{ ($anggota->goldar == 'A-') ? 'selected' : '' }}>A-</option>
                        <option value="B+"
                            {{ ($anggota->goldar == 'B+') ? 'selected' : '' }}>B+</option>
                        <option value="B-"
                            {{ ($anggota->goldar == 'B-') ? 'selected' : '' }}>B-</option>
                        <option value="O+">
                            {{ ($anggota->goldar == 'O+') ? 'selected' : '' }}O+</option>
                        <option value="O-"
                            {{ ($anggota->goldar == 'O-') ? 'selected' : '' }}>O-</option>
                        <option value="AB+"
                            {{ ($anggota->goldar == 'AB+') ? 'selected' : '' }}>AB+</option>
                        <option value="AB-" 
                            {{ ($anggota->goldar == 'AB-') ? 'selected' : '' }}>AB-</option>
                    </select>
                </div>
